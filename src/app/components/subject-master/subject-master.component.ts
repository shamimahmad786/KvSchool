...
export class SubjectMasterComponent implements OnInit,AfterViewInit {
  ...
  testData = { "sno": "", "subjectCode": "", "subjectName": "", "status": "","statusType": "","id":"" }
  ...
  ngOnInit(): void {
       ...
       if(res.length>0){
           for (let i = 0; i < res.length; i++) {
            ...
            if(res[i].status ==false )
             ...
             this.testData = { "sno": "", "subjectCode": "", "subjectName": "", "status": "","statusType": "","id":"" };
   ...
   exportexcel(){
    ...
    const workSheet = workBook.addWorksheet('SubjectMaster');
    const excelData = [];
    const ws1 = workSheet.addRow(['', 'SUBJECT MASTER', '']);
   ...
   const ws = workSheet.addRow(['Subject Code', 'Subject Name', 'Status']);
   workSheet.getRow(2).font = { name: 'Arial', family: 4, size: 10, bold: true };
    ...
    this.listDesignation.forEach((item) => {
      const row = workSheet.addRow([item.subjectCode, item.subjectName,item.statusType]);
    });
    workBook.xlsx.writeBuffer().then((data) => {
      let blob = new Blob([data], {
        type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      });
      saveAs(blob, 'SubjectMaster.xlsx');
    });
